...
def main():
    ...
    for sc in script["scenes"]:
        ...
        if not text.strip():
            continue                      # silent scenes (montage) — no TTS
        if os.path.exists(os.path.join(MIX, f"{sid}.wav")) and not only:
            continue                      # already generated; don't re-bill
        if only and sid not in only.split(","):
            continue
        mp3 = os.path.join(NARR, f"{sid}.mp3")
        tts(text, mp3, key)
        wav = os.path.join(MIX, f"{sid}.wav")
        subprocess.run(["ffmpeg", "-y", "-v", "error", "-i", mp3, "-ar", "48000", "-ac", "1", "-c:a", "pcm_s16le", wav], check=True)

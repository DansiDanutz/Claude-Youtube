...
TOTAL = dur(VID)
# fades at chapter starts, computed from the concat's actual segment durations
import json as _json
CHAPTERS = ["ZStakes", "ZTrap", "ZNervix", "ZDexterIntro", "ZZmarty", "ZSemeClaw", "ZFlywheel", "ZHumanBeat", "ZMissing", "ZOutro"]
SEGS = ["ZDollar","ZIntro","ZStakes","ZTrap","ZTrust","ZNervix","ZWhoJoins","ZFrontDoor","ZDexterIntro","Z14k","ZCards","ZPoker","ZZmarty","ZSienna","ZTeacher","ZSemeClaw","ZAdsLoop","ZFlywheel","ZSpin","ZHumanBeat","ZMissing","ZPromise","ZOutro"]
_starts, _t = {}, 0.0
for _s in SEGS:
    _starts[_s] = _t
    _t += dur(f"{OUT}/{_s}.mp4")
FADES = [_starts[c] for c in CHAPTERS]
_c, CUTS = 0.0, [0.0]
while _c + 150 < TOTAL:
    _c += 110
    while any(abs(_c - t) <= 2.5 or abs(_c - (t - 0.4)) <= 2.5 for t in FADES):
        _c += 5
    CUTS.append(_c)
CUTS.append(TOTAL)
for c in CUTS[1:-1]:
...
with open(lst, "w") as fh:
    for c in chunks:
        fh.write(f"file '{c}'\n")
out = f"{EP}/danslab-episode-11-flywheel-{TAG.upper()}-v2.mp4"

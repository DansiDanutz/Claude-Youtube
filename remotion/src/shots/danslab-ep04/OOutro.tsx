...
// Ep04 22 — outro: No.04 The Overseer → tease Hermes/The Brain → subscribe →
// Build. Ship. Repeat. VO 1.0s (17.9s).
export const compositionConfig = { id: 'OOutro', durationInSeconds: 21, fps: 30, width: 1920, height: 1080 };

const OOutro: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ fontFamily: DL_SANS, opacity: fade }}>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        ...
        <div style={{ opacity: nameOp, fontFamily: DL_MONO, fontSize: 24, letterSpacing: 8, color: DL.red, marginTop: 28 }}>DANSLAB · NO. 04</div>
        <div style={{ opacity: nameOp, fontFamily: DL_SERIF, fontWeight: 500, fontSize: 92, color: DL.text, marginTop: 8 }}>The <span style={{ fontStyle: 'italic', color: DL.gold }}>Overseer</span></div>

        <div style={{ opacity: nextOp, marginTop: 44, fontFamily: DL_SERIF, fontStyle: 'italic', fontSize: 40, color: DL.warm }}>
          Next: <span style={{ color: DL.text }}>The Brain</span> — the mind that turns a fleet of rebels into a team. Its name is <span style={{ color: DL.sky }}>Hermes.</span>
        </div>
        ...
        <div style={{ opacity: subOp, marginTop: 36, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ fontFamily: DL_SANS, fontSize: 28, color: DL.dim }}>watch a company learn to run itself</span>
        </div>
